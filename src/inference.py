...
from src.schemas import (
    PropertyInput,
)  # Ensure relative import if running via 'cd .. && uvicorn src.app:app'
...
try:
    ...
    print(
        f"Failed to load model artifacts. Ensure the 'model/' directory exists and contains all files. Error: {e}"
    )
    raise
...
def predict_price(data: PropertyInput) -> dict:
    ...
    price_by_subarea = sub_area_map.get(
        sub_area_clean, np.mean(list(sub_area_map.values()))
    )
    ...
    price_by_amenities = amenities_map.get(
        amenity_score, np.mean(list(amenities_map.values()))
    )
...
def get_model_info() -> dict:
    ...
    return {
        ...
        "interval_margin": round(
            interval_est["z_score"] * interval_est["residual_std"], 2
        ),
    }
